...
.always(function() {       // 這是 Promise-Like 的方法，比較現代的寫法
    console.log('always');
});
```
...
- `always`：較新的 Promise-Like 風格寫法，程式碼更容易維護和串接
...
是的，正確！挑一個來寫即可，建議使用 `.always()`，因為這是較新的 Promise-Like 風格寫法。
