...
**$.ajax() 依 jQuery 版本，回傳 jqXHR (Promise-like) 或 jQuery Promise (Deferred / 非原生 Promise)**

==不管是 jqXHR 還是 jQuery Promise，方法幾乎是一樣的，少許的差異：==
...
- `.always(function (data, textStatus, jqXHR) { })` ($.ajax 成功時)
- `.always(function (jqXHR, textStatus, errorThrown) { })` ($.ajax 失敗時)
- `.finally(function () { })` ( ==僅 jQuery Promise 支援，無參數== )

### await 行為

※ 注意 await 時，會把 jqXHR、jQuery Promise 轉成原生 Promise，所以只會有一個參數：

- `await jqXHR(成功時)` => 取得 `data`
- `await jQueryPromise(成功時)` => 取得 `data`
- `await jqXHR(失敗時)` => 取得 `jqXHR` ✅
- `await jQueryPromise(失敗時)` => 取得 `jqXHR` ✅

**失敗時的 await 需要用 try/catch**：

```javascript
try {
    const data = await $.ajax({...}); // 成功：得到 data
} catch (jqXHR) {
    console.log(jqXHR.status); // 失敗：得到 jqXHR
}
```
