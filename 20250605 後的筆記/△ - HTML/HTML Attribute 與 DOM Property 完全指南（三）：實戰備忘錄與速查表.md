...
**重要邊界**：「切換選項或修改 `value` 時，操作僅存在於 DOM 實例的狀態中，不會變更 HTML 標籤屬性（如 `selected` 屬性不會跟著跑）；但若使用 `innerHTML` 進行寫入，則會直接覆蓋並變更 HTML 的原始結構。」
...
**<font color="#ffff00">★Attribute 與 Property 雙向連動（option 的特殊行為）</font>：**
...
關於 checkbox 未勾選的送出行為，許多開發者可能不知道：後端收不到這個欄位，不是收到 `false` 或空字串，<font color="#ff0000">而是根本沒有這個 key</font>。這意味著後端需要用「欄位存不存在」來判斷是否勾選，而不是判斷值是 `true` 還是 `false`。
...
```javascript
$('#memo').empty()     // ❌ 只清了初始浮水印，使用者打的字還在 (原因是「原始狀態、目前狀態」已經發生了脫鈎，innerHTML/textContent 不連動 value 了)
