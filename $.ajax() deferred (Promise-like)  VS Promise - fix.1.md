...
這是我們討論中最重要的發現之一。經過實際測試，我們確認了 jQuery 傳統方法與現代 Promise 方法在資料傳遞上有根本性的不同。

### done() 方法的觀察者模式

**重要發現：多個 done() 方法都接收相同的原始資料，不會形成資料傳遞鏈**
...
.done(function(users) {
    ...
    return processedUsers; // 這個回傳值會被忽略！
})
.done(function(users) {
...
這種行為類似於「事件監聽器」的概念，每個 `done()` 都是獨立地「監聽」同一個成功事件，並接收相同的原始資料。
...
.then(function(users) {
    ...
    return $.ajax('/api/users/' + firstUserId + '/details'); // 回傳另一個 Promise
})
.then(function(userDetails) {
    // 這裡接收到的是第二個 AJAX 請求的結果，不是 Promise 物件本身
...
.fail(function(jqXHR, textStatus, errorThrown) {
    ...
    return defaultData; // 這個回傳值無法改變 Promise 的 rejected 狀態！
})
.then(function(data) {
    console.log('這個 then 不會執行'); // 不會執行，因為 Promise 仍然是 rejected 狀態
})
.catch(function(error) {
...
這個行為經常讓開發者感到困惑，因為它與原生 Promise 的 catch() 行為完全不同。
...
傳統的 `done()`, `fail()`, `always()` 方法不會自動包裝回傳值。它們的回傳值要麼被忽略（如 done() 和 fail()），要麼在特定情況下被使用：
