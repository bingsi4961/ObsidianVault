...
function fetchUserDataWithPromise(userId) {
    return fetch(`/api/users/${userId}`)
		🔥🔥🔥
		// then() 告訴 JavaScript「等 fetch 完成後，再執行這個函式」
		// 因為 fetch 完成，then() 可以從 Promise<Response>，取得 Response
...
fetchUserDataWithAsync(123) <= Promise<Object>
await fetchUserDataWithAsync(123) <= Object

若 fetchUserDataWithAsync(123) 回傳 Promise<Error> 時
await fetchUserDataWithAsync(123) 會拋出例外，因為已可從 Promise.reject(error) 中取得 error，故發生例外
