...
**關鍵觀念**：即使經過多個 async 方法，在碰到第一個「真正需要等待」的操作前，都是同一個執行緒在執行
...
**重要觀察**：Thread B 會盡可能把所有後續工作一次完成，不會每個 await 都換執行緒
    ...
    public void MoveNext()
    {
        ...
        try
        {
            switch (state)
            {
                ...
                case 0: // 從 await 恢復
                    delayAwaiter.GetResult(); // 檢查是否有異常
                    break;
            }
