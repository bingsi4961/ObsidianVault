...
【補充說明】原始討論中曾提到 EF Core 從 6.0 開始已原生支援 SQL Server 時態表，能用 LINQ 寫出對應 `AS OF` / `FROM...TO` 的歷史查詢語法，但當時的討論並未展開實際寫法就轉向了其他問題，所以這裡先保留一個開放性的提醒：若你日後要在 GamingPortal（.NET 8.0 + EF Core 8.0.22）專案中查詢時態表，可以留意 EF Core 官方文件中 Temporal Tables 相關的查詢 API，屆時我們可以再另開一篇專門討論 EF Core 寫法的文章，避免在這裡憑印象帶過。

---

## 全系列小結

三篇文章串起來，你應該已經掌握了時態表的完整生命週期：

1. **建立**：`ALTER TABLE` 兩步驟、欄位與關鍵字意義、INSERT/UPDATE/DELETE 實際運作、防止歷史表被灌爆
2. **查詢**：UTC 時區轉換、`AS OF` 快照查詢、`FROM...TO` / `BETWEEN...AND` / `CONTAINED IN` 的重疊 vs 完全包含邏輯
3. **還原與維運**：用 `UPDATE` / `INSERT` 正確還原資料、絕不關閉版本控制手動搬資料、索引與保留原則維持效能

下次遇到「這筆資料上週被誰改掉了」這種問題，你應該已經能從容應對了！