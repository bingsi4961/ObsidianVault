...
> 閱讀順序：本篇為系列第 1 篇，建議依序閱讀 01 → 02 → 03。

嗨，這篇我們從頭帶你認識「系統版本時態表（System-Versioned Temporal Table）」。這是 SQL Server 2016 之後才有的功能，如果你之前沒碰過，別緊張，我們一步一步來。
...
理解了語法之後，最重要的是搞懂「資料異動時，主表跟歷史表分別會發生什麼事」。這部分結合了筆記整理與實際 Q&A 討論釐清的細節。
...
現在我們知道資料是怎麼被安全、準確地寫進時態表的了。接下來的重頭戲是「查詢」——當老闆問你「這筆訂單昨天下午 3 點時的狀態是什麼」，你要怎麼撈出正確的資料？這部分我們留到 **第二篇：時態表查詢技巧** 詳細說明。